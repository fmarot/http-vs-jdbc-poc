...
public class FileController {
	...
	public FileController(LargeFileService lfService) {
		this.lfService = lfService;
	}

	@PostMapping(value = Endpoints.upload,
			consumes = MediaType.APPLICATION_OCTET_STREAM_VALUE,
			produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<Long> upload(
			@PathVariable(Endpoints.UploadMethods.filename_var) String filename,
			@PathVariable(Endpoints.UploadMethods.filesize_var) long filesize,
